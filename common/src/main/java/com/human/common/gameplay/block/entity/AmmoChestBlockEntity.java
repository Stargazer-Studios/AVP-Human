...
public class AmmoChestBlockEntity extends RandomizableContainerBlockEntity implements LidBlockEntity {
    ...
    public AmmoChestBlockEntity(BlockPos pos, BlockState blockState) {
        super(HumanBlockEntityTypes.AMMO_CHEST.get(), pos, blockState);
        ...
        this.openersCounter = new ContainerOpenersCounter() {
            ...
            protected void openerCountChanged(Level level, BlockPos pos, BlockState state, int count, int openCount) {
                level.blockEvent(AmmoChestBlockEntity.this.worldPosition, HumanBlocks.AMMO_CHEST.get(), 1, openCount);
            }
    ...
    private boolean isValidForChest(ItemStack stack) {
        return stack.isEmpty() || stack.is(HumanItemTags.AMMO_ITEMS);
    }
    ...
    public boolean hasAmmo() {
        return !this.itemStacks.isEmpty() && this.itemStacks.stream().anyMatch(item -> item.is(HumanItems.MEDIUM_BULLET.get()));
    }

    public boolean consumeAmmo(int count) {
        for (ItemStack itemStack : this.itemStacks) {
            if (itemStack.is(HumanItems.MEDIUM_BULLET.get())) {
