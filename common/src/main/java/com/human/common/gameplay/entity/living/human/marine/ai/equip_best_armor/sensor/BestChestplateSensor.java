...
public class BestChestplateSensor {
    ...
    public static Option<ArmorStrategyResult<? extends ItemTarget>> sense(LivingEntity ignored, ReadableWorldState worldState) {
        var equippedOption = worldState.getOrDefault(EquipBestArmorSensors.BEST_CHESTPLATE_EQUIPPED.key(), Option.none());
        var inventoryOption = worldState.getOrDefault(EquipBestArmorSensors.BEST_CHESTPLATE_IN_INVENTORY.key(), Option.none());
        var worldOption = worldState.getOrDefault(EquipBestArmorSensors.BEST_CHESTPLATE_IN_WORLD.key(), Option.none());
