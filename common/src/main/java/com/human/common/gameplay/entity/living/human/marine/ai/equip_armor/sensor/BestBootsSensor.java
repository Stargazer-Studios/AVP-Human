...
public class BestBootsSensor {
    ...
    public static Option<ArmorStrategyResult<? extends ItemTarget>> sense(LivingEntity ignored, ReadableWorldState worldState) {
        var equippedOption = worldState.getOrDefault(EquipArmorSensors.BEST_BOOTS_EQUIPPED.key(), Option.none());
        var inventoryOption = worldState.getOrDefault(EquipArmorSensors.BEST_BOOTS_IN_INVENTORY.key(), Option.none());
        var worldOption = worldState.getOrDefault(EquipArmorSensors.BEST_BOOTS_IN_WORLD.key(), Option.none());
