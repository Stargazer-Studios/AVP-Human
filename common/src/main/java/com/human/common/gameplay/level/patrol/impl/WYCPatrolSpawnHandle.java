...
import com.human.common.gameplay.level.patrol.decorator.gear.WYCGearDecorator;
...
public class WYCPatrolSpawnHandle {
    ...
    private WYCPatrolSpawnHandle() {
        this.spawner = PatrolSpawner.builder()
            ...
            .withSpawnPositionSelector(player -> {
                ...
                if (position == null || !player.level().getBiome(position).is(HumanBiomeTags.HAS_WY_COMMANDO_PATROLS)) {
                    return null;
                }
        ...
        this.ticker = PatrolSpawnerTicker.builder()
            ...
            .withTiming(PatrolSpawnTimings.WY_COMMANDO_PATROLS)
            .build(spawner);
    }
    ...
    private void spawn(Level level, Player player, BlockPos.MutableBlockPos mutableBlockPos) {
        ...
        for (var marine : spawnedMarines) {
            WYCGearDecorator.INSTANCE.decorate(level, marine);
        }
    }
}
