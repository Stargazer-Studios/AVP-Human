...
public class HumanPropertyAccess {

    private static final Path PATH = BLibAPI.getGameDirectory().resolve(Path.of("avp_human.properties"));

    public static final HumanPropertyAccess INSTANCE = new HumanPropertyAccess();

    private BLibPropertyContainer propertyContainer;

    public void save() {
        BLibProperties.save(PATH, getOrCreatePropertyContainer(), HumanPropertySchema.SCHEMA);
    }

    private BLibPropertyContainer getOrCreatePropertyContainer() {
        if (this.propertyContainer == null) {
            var result = BLibProperties.load(PATH, HumanPropertySchema.SCHEMA);
                ...
                .inspectErr(loadError -> {
                    switch (loadError) {
                        case BLibProperties.LoadError.DoesNotExist ignored -> {
                            this.propertyContainer = new BLibPropertyContainer(new HashMap<>(), HumanPropertySchema.SCHEMA);
                            save();
                        }
